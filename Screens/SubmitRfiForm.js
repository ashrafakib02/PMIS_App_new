...
export default function SubmitRfiForm() {
  ...
  const SubmitFn = () => {
    if (date2 < currentDate || date2 == currentDate) {
      ...
      activityString.length == 0 ||
      activityString == 'Select Activity'
    ) {
      alert('Please Select Activity!')
    } else if (
      (activityString == 'Other Activities' &&
        activityDetailString == 'NULL') ||
      (activityString == 'Other Activities' &&
        activityDetailString.value.length == 0)
    ) {
        ...
        <ScrollView style={{ flex: 1, margin: 2 }}>
          ...
          <View style={{ flex: 1, marginLeft: 5 }}>
            {activityString == 'Other Activities' ? (
              <TextInput
                ...
                onChangeText={(text) =>
                  setActivityDetailString({ value: text, error: '' })
                }
              />
            ) : null}
          </View>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.titleBoxBig}>
              Location (in Details) with Chainage
            </Text>
            <TextInput
              style={[tw`text-indigo-700`, styles.DataBoxBig]}
